    ...
    def send_command(self, event_attributes):
        ...
        if action == str(PubSub.BuildActions.CYBER_ARENA_ATTACK.value):
                    ...
                    for workspace in workspaces:
                        ...
                        if not self.debug:
                            PubSubManager(topic=str(PubSub.Topics.CYBER_ARENA.value)).msg(
                                ...
                                build_type=str(DatastoreKeyTypes.FIXED_ARENA_WORKSPACE.value)
                            )
                        else:
                            CyberArenaAgent(build_id=w_id, debug=self.debug).send_command(event_attributes)
            ...
            if build_type == BuildConstants.BuildType.FIXED_ARENA_WORKSPACE:
                ...
                if not self.debug:
                    command = PubSubManager(topic=agent_topic).msg(build_id=self.build_id, attack_id=attack_obj['id'],
                                                                   attack_args=attack_obj['args'],
                                                                   attack_module=attack_obj['module'])
                else:
                    print(f'{agent_topic} ==> {self.build_id}; {attack_obj["id"]}; {attack_obj["args"]}; {attack_obj["module"]}')
