...
class IoTApp:
    BUILD_CLOUD_RUN_COMMAND = "gcloud builds submit . --tag gcr.io/{project}/gencyber_iot"
    DEPLOY_CLOUD_RUN_COMMAND = "gcloud run deploy gencyber-iot --image gcr.io/{project}/gencyber_iot --memory=1024Mi " \
                               "--platform=managed --region={region} --allow-unauthenticated " \
                               "--service-account=cyberarena-service@{project}.iam.gserviceaccount.com"
    MAP_DNS_COMMAND = "gcloud beta run domain-mappings create --service gencyber-iot --domain={dns} --region={region}"
    DEPLOY_CLOUD_FUNCTION_COMMAND = "gcloud functions deploy --quiet gencyber-iot " \
                                    "--region={region} --memory=1024Mi " \
                                    "--entry-point=iot_device_cloud_function " \
                                    "--runtime=python311 --source=\".\" " \
                                    "--service-account=cyberarena-service@{project}.iam.gserviceaccount.com " \
                                    "--timeout=540s " \
                                    "--trigger-topic=cybergym-telemetry"

    CLOUD_SCHEDULER_NAME = "iot-device-hourly-maintenance"
    SCHEDULE = "*/3 * * * *"
    ATTRIBUTES = json.dumps({'action': PubSub.IotActions.MAINTENANCE.value})
    CLOUD_SCHEDULER_COMMAND = f"gcloud scheduler jobs create pubsub {CLOUD_SCHEDULER_NAME} " \
                              f"--schedule=\"{SCHEDULE}\" --topic=cybergym-telemetry --message-body=Hello! " \
                              f"--attributes=maintenance=True --location=us-central1"
